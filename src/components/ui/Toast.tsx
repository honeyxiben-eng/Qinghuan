...
export function ToastContainer(){
  ...
  return <div className='fixed bottom-6 right-6 z-[9999] flex flex-col gap-3 max-w-sm'>
    {toasts.map((t,i)=><div key={t.id} className='flex items-center gap-3 px-5 py-3.5' style={{background:'rgba(24,24,32,0.92)',border:'1px solid var(--glass-border-strong)',borderRadius:'var(--r-lg)',boxShadow:'var(--s-lg)',backdropFilter:'blur(24px)',borderLeft:'3px solid '+cfg[t.type].dot,animation:'slideInRight 0.3s var(--ease-spring) both '+(i*0.05)+'s'}}>
