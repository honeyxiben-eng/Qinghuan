...
export default function LabPage() {
  ...
  const [lw, setLw] = useState(""); const [ld, setLd] = useState(new Date().toISOString().split("T")[0]); const [submitting, setSubmitting] = useState(false)
  ...
  fetchRef.current = () => { setLoading(true); getLabData({ region: region || undefined, lineId: line ? WELL_LINES.find(ll => ll.shortName === line)?.id : undefined, search: search || undefined, dateFrom: df || undefined, dateTo: dt || undefined, page, pageSize: PS }).then((r: any) => { setData(r.data); setTotal(r.total); setStats(r.stats || null) }).finally(() => setLoading(false)) }
  ...
  const cf = () => { setLw(""); setLd(new Date().toISOString().split("T")[0]); setLv(""); setLn(""); setLp(""); setLs(""); setLk(""); setLm(""); setLc(""); setLo(""); setLa(""); setLb(""); setLi(""); setLna(""); setIs([]) }
  const sub = async (e: React.FormEvent) => { e.preventDefault(); if (submitting) return; if (!lw) { addToast("请填写井号", "warning"); return } const vResult = labCreateSchema.safeParse({ wellId: lw, testDate: ld || undefined, viscosity: lv ? parseFloat(lv) : undefined, density: ln ? parseFloat(ln) : undefined, ph: lp ? parseFloat(lp) : undefined, salinity: ls ? parseFloat(ls) : undefined, kPlus: lk ? parseFloat(lk) : undefined, mg2Plus: lm ? parseFloat(lm) : undefined, clMinus: lc ? parseFloat(lc) : undefined, so42Minus: lo ? parseFloat(lo) : undefined, ca2Plus: la ? parseFloat(la) : undefined, b2o3: lb ? parseFloat(lb) : undefined, liPlus: li ? parseFloat(li) : undefined, naPlus: lna ? parseFloat(lna) : undefined }); if (!vResult.success) { addToast(vResult.error.issues[0].message, "warning"); return } setSubmitting(true); try { await createLabData(vResult.data); addToast("已保存", "success"); cf() } catch (e: any) { addToast("保存失败:" + e.message, "error") } finally { setSubmitting(false) } }
  ...
  const doExp = async () => {
    try {
      ...
      exportXLSX({ filename: "化验数据", headers: ["井号", "日期", "粘度", "比重", "pH", "矿化度", "K⁺", "Mg²⁺", "Cl⁻", "SO₄²⁻", "Ca²⁺", "B₂O₃", "Li⁺", "Na⁺"], rows: rows.map((r: any) => [r.wellId, fmtDate(r.testDate), fmt(r.viscosity, P.VISCOSITY), fmt(r.density, P.DENSITY), fmt(r.ph, P.PH), fmt(r.salinity, P.SALINITY), fmt(r.kPlus, P.ION), fmt(r.mg2Plus, P.ION), fmt(r.clMinus, P.ION), fmt(r.so42Minus, P.ION), fmt(r.ca2Plus, P.ION), fmt(r.b2o3, P.ION_B2O3), fmt(r.liPlus, P.ION_LI), fmt(r.naPlus, P.ION)]) })
      addToast("已导出 " + rows.length + " 条", "success")
    } catch (e: any) { addToast("导出失败: " + e.message, "error") }
  }
  const doExpCSV = async () => { try { const all = await getLabData({ region: region || undefined, lineId: line ? WELL_LINES.find(l => l.shortName === line)?.id : undefined, search: search || undefined, dateFrom: df || undefined, dateTo: dt || undefined, pageSize: 99999 }); const rows = (all as any).data || []; if (!rows.length) { addToast("无数据可导出", "warning"); return } exportCSV("化验数据", ["井号", "日期", "粘度", "比重", "pH", "矿化度", "K⁺", "Mg²⁺", "Cl⁻", "SO₄²⁻", "Ca²⁺", "B₂O₃", "Li⁺", "Na⁺"], rows.map((r: any) => [r.wellId, fmtDate(r.testDate), fmt(r.viscosity, P.VISCOSITY), fmt(r.density, P.DENSITY), fmt(r.ph, P.PH), fmt(r.salinity, P.SALINITY), fmt(r.kPlus, P.ION), fmt(r.mg2Plus, P.ION), fmt(r.clMinus, P.ION), fmt(r.so42Minus, P.ION), fmt(r.ca2Plus, P.ION), fmt(r.b2o3, P.ION_B2O3), fmt(r.liPlus, P.ION_LI), fmt(r.naPlus, P.ION)])); addToast("CSV 已导出", "success") } catch (e: any) { addToast("导出失败: " + e.message, "error") } }
  ...
  return <div className="page-container">
    ...
    {tab === "browse" && <>
      ...
      <div className="overflow-hidden">
        ...
        <DataStats data={data} total={total} stats={stats} cols={[{ key: "kPlus", label: "K", decimals: 3 }, { key: "liPlus", label: "Li", decimals: 4 }, { key: "density", label: "比重", decimals: 4 }, { key: "salinity", label: "矿化度", decimals: 3 }]} />
    ...
    {tab === "entry" && <div className="card p-5 rise"><form onSubmit={sub}>
      <div className="flex items-center justify-center gap-2 mb-4">
        <label className="import-btn cursor-pointer">📥 导入Excel<input type="file" accept=".xlsx,.xls" className="hidden" onChange={async e => { const f = e.target.files?.[0]; if (!f) return; try { const XLSX = await import("xlsx"); const T = await import("@/lib/templates"); const d = new Uint8Array(await f.arrayBuffer()); const wb = XLSX.read(d, { type: "array" }); const raw = XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]], { header: 1, raw: false }) as any[][]; const rows = raw.filter((r: any[]) => r.some((c: any) => c !== "")); const find = T.colFinder(rows[0]); const A = T.LAB_ALIASES; const cell = (r: any[], al: string[]) => { const i = find(al); return i >= 0 ? r[i] : undefined }; const recs = rows.slice(1).map((r: any) => ({ wellId: String(cell(r, A.wellId) || "").trim(), testDate: T.parseDate(cell(r, A.testDate)), viscosity: T.num(cell(r, A.viscosity)), density: T.num(cell(r, A.density)), ph: T.num(cell(r, A.ph)), salinity: T.num(cell(r, A.salinity)), kPlus: T.num(cell(r, A.kPlus)), mg2Plus: T.num(cell(r, A.mg2Plus)), clMinus: T.num(cell(r, A.clMinus)), so42Minus: T.num(cell(r, A.so42Minus)), ca2Plus: T.num(cell(r, A.ca2Plus)), b2o3: T.num(cell(r, A.b2o3)), liPlus: T.num(cell(r, A.liPlus)), naPlus: T.num(cell(r, A.naPlus)) })).filter((x: any) => x.wellId); const result = await importLabDatas(recs); if (result.success) { addToast("导入" + result.count + "条" + (result.skipped ? "，跳过" + result.skipped + "条(井号不存在)" : ""), "success"); fetch() } else { addToast("导入失败:" + result.error, "error") } } catch (err: any) { addToast("导入异常:" + (err?.message || ""), "error") } e.target.value = "" }} /></label>
        <button type="button" onClick={() => { import("@/lib/templates").then(m => m.downloadTemplate(m.LAB_TPL, "化验数据导入模板", m.LAB_SAMPLE)) }} className="import-btn">📋 下载模板</button>
      ...
      <div className="flex items-end gap-2 overflow-x-auto pb-2" style={{scrollbarWidth:'thin'}}>
        <div className="relative flex-shrink-0"><Input label="井号" value={lw} onChange={e => hs(e.target.value)} placeholder="C05001" w={105} autoComplete="off" />{is.length > 0 && <div className="absolute z-50 top-full left-0 right-0 mt-1 pop" style={{ background: "var(--surface-3)", border: "1px solid var(--glass-border-strong)", borderRadius: "var(--r-md)", boxShadow: "var(--s-lg)", backdropFilter: "blur(20px)" }}>{is.map(id => <div key={id} className="px-3 py-2 text-[12px] hover:bg-[var(--surface-1)] cursor-pointer" style={{ fontFamily: "var(--font-mono)" }} onClick={() => { setLw(id); setIs([]) }}>{id}</div>)}</div>}</div>
        <Input label="日期" value={ld} onChange={e => setLd(e.target.value)} placeholder="2024-02-28" w={115} />
        <Input label="粘度" value={lv} onChange={e => setLv(e.target.value)} w={72} />
        <Input label="比重" value={ln} onChange={e => setLn(e.target.value)} placeholder="1.2345" w={85} />
        <Input label="pH" value={lp} onChange={e => setLp(e.target.value)} placeholder="7.00" w={68} />
        <Input label="矿化度" value={ls} onChange={e => setLs(e.target.value)} w={85} />
        <Input label="K⁺" value={lk} onChange={e => setLk(e.target.value)} w={72} />
        <Input label="Mg²⁺" value={lm} onChange={e => setLm(e.target.value)} w={72} />
        <Input label="Cl⁻" value={lc} onChange={e => setLc(e.target.value)} w={72} />
        <Input label="SO₄²⁻" value={lo} onChange={e => setLo(e.target.value)} w={75} />
        <Input label="Ca²⁺" value={la} onChange={e => setLa(e.target.value)} w={72} />
        <Input label="B₂O₃" value={lb} onChange={e => setLb(e.target.value)} w={72} />
        <Input label="Li⁺" value={li} onChange={e => setLi(e.target.value)} w={72} />
        <Input label="Na⁺" value={lna} onChange={e => setLna(e.target.value)} w={72} />
