...
export function createWell(d: any) {
  const result = run(`INSERT INTO WellInfo(wellId,lineId,completionDate,technology,techNote,wellSize,initialWaterLevel,designDepth,coordX,coordY)
    VALUES(?,?,?,?,?,?,?,?,?,?)
  ...
  auditLog({ action: 'create', entityType: 'well', entityId: d.wellId, newValues: d })
  return result
}
export function updateWell(id: string, d: any) {
  const existing = one('SELECT * FROM WellInfo WHERE wellId = ?', [id])
  ...
  p.push(id)
  const result = run("UPDATE WellInfo SET " + f.join(",") + " WHERE wellId=$" + (n + 1), p)
  auditLog({ action: 'update', entityType: 'well', entityId: id, oldValues: existing, newValues: d })
  return result
}
export function deleteWells(ids: string[]) {
  const result = run("DELETE FROM WellInfo WHERE wellId IN(" + ids.map((_, i) => "$" + (i + 1)).join(",") + ")", ids)
  for (const id of ids) {
    auditLog({ action: 'delete', entityType: 'well', entityId: id })
  }
  return result
}
...
export function importWells(recs: any[]): { success: boolean; count: number; skipped: number; error?: string } {
  try {
