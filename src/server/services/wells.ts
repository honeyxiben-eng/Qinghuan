...
export function getWells(o?: { region?: string; lineId?: number; search?: string; page?: number; pageSize?: number }): Paginated<any> {
  ...
  const total = (one<{ c: number }>(`SELECT COUNT(*) as c FROM WellInfo wi JOIN WellLineInfo wl ON wi.lineId=wl.id WHERE ${w}`, pr))?.c || 0
  const data = all(`SELECT wi.*,wl.name as lineName,wl.shortName,wl.region FROM WellInfo wi JOIN WellLineInfo wl ON wi.lineId=wl.id WHERE ${w} ORDER BY wl.regionSeq,wi.wellId LIMIT $${pr.length + 1} OFFSET $${pr.length + 2}`, [...pr, ps, (p - 1) * ps])
  return { data, total, page: p, pageSize: ps, totalPages: Math.ceil(total / ps) }
}

export function getWellById(id: string) { return one("SELECT wi.*,wl.name as lineName,wl.shortName,wl.region FROM WellInfo wi JOIN WellLineInfo wl ON wi.lineId=wl.id WHERE wi.wellId=?", [id]) }
export function createWell(d: any) {
  return run(`INSERT INTO WellInfo(wellId,lineId,completionDate,technology,techNote,wellSize,initialWaterLevel,designDepth,coordX,coordY)
    VALUES(?,?,?,?,?,?,?,?,?,?)
...
export function updateWell(id: string, d: any) { const f: string[] = []; const p: any[] = []; let n = 0; for (const k of ["completionDate", "technology", "techNote", "wellSize", "initialWaterLevel", "designDepth", "coordX", "coordY"]) { if (d[k] !== undefined) { n++; f.push(k + "=$" + n); p.push(k === "completionDate" ? (d[k] ? fmtDate(d[k]) : null) : (d[k] ?? null)) } } if (!f.length) return { changes: 0 }; p.push(id); return run("UPDATE WellInfo SET " + f.join(",") + " WHERE wellId=$" + (n + 1), p) }
export function deleteWells(ids: string[]) { return run("DELETE FROM WellInfo WHERE wellId IN(" + ids.map((_, i) => "$" + (i + 1)).join(",") + ")", ids) }
export function exportWells() { return all("SELECT wi.*,wl.name as lineName,wl.shortName,wl.region FROM WellInfo wi JOIN WellLineInfo wl ON wi.lineId=wl.id ORDER BY wl.regionSeq,wi.wellId") }
export function getLines() { return all("SELECT * FROM WellLineInfo ORDER BY regionSeq,id") }

export function importWells(recs: any[]): { success: boolean; count: number; error?: string } {
  try {
    const m = new Map<string, number>(); (all<any>("SELECT id,shortName,name FROM WellLineInfo")).forEach((l: any) => { m.set(l.shortName, l.id); m.set(l.name, l.id) })
    const miss = [...new Set(recs.map((r: any) => r.shortName).filter((s: any) => s && !m.has(s)))]
    if (miss.length) return { success: false, count: 0, error: "井采线不存在: " + miss.join(", ") }
    const n = tx(() => { let c = 0; for (const r of recs) { run(`INSERT INTO WellInfo(wellId,lineId,completionDate,technology,techNote,wellSize,initialWaterLevel,designDepth,coordX,coordY) VALUES(?,?,?,?,?,?,?,?,?,?) ON CONFLICT(wellId) DO UPDATE SET lineId=excluded.lineId,completionDate=COALESCE(excluded.completionDate,WellInfo.completionDate),technology=COALESCE(excluded.technology,WellInfo.technology),techNote=COALESCE(excluded.techNote,WellInfo.techNote),wellSize=COALESCE(excluded.wellSize,WellInfo.wellSize),initialWaterLevel=COALESCE(excluded.initialWaterLevel,WellInfo.initialWaterLevel),designDepth=COALESCE(excluded.designDepth,WellInfo.designDepth),coordX=COALESCE(excluded.coordX,WellInfo.coordX),coordY=COALESCE(excluded.coordY,WellInfo.coordY)`, [r.wellId, m.get(r.shortName) || r.lineId || 0, r.completionDate ? fmtDate(r.completionDate) : null, r.technology || null, r.techNote || null, r.wellSize || null, r.initialWaterLevel ?? null, r.designDepth ?? null, r.coordX ?? null, r.coordY ?? null]); c++ } return c })
