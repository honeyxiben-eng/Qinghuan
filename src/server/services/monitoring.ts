 ...
 */
function fillFromLast(d: any) {
  const last = one<any>("SELECT * FROM DynamicMonitoring WHERE wellId=? ORDER BY collectDate DESC, id DESC LIMIT 1", [d.wellId])
  if (!last) return d
...
export function updateMonitoring(id: number, d: any) {
  ...
  p.push(id)
  const result = run("UPDATE DynamicMonitoring SET " + f.join(",") + " WHERE id=$" + (n + 1), p)
  auditLog({ action: 'update', entityType: 'monitoring', entityId: String(id), oldValues: existing, newValues: d })
  return result
}
export function deleteMonitorings(ids: number[]) {
  const result = run("DELETE FROM DynamicMonitoring WHERE id IN(" + ids.map((_, i) => "$" + (i + 1)).join(",") + ")", ids)
  for (const id of ids) {
    auditLog({ action: 'delete', entityType: 'monitoring', entityId: String(id) })
  }
  return result
}
...
export function importMonitorings(recs: any[]): { success: boolean; count: number; skipped: number; error?: string } {
  try {
    ...
    const n = tx(() => { let c = 0; for (const r of valid) { const filled = fillFromLast({...r, wellId: String(r.wellId).trim()}); run("INSERT INTO DynamicMonitoring(wellId,collectDate,staticWater,dynamicWater,wellDepth,flowRate,pumpDepth,pumpFlow,motorPower,manufacturer,status,faultNote)VALUES(?,?,?,?,?,?,?,?,?,?,?,?)", [filled.wellId, filled.collectDate ? fmtDate(filled.collectDate) : null, filled.staticWater ?? null, filled.dynamicWater ?? null, filled.wellDepth ?? null, filled.flowRate ?? null, filled.pumpDepth ?? null, filled.pumpFlow ?? null, filled.motorPower ?? null, filled.manufacturer || null, filled.status || "normal", filled.faultNote || null]); c++ } return c })
